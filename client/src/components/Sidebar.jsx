...
const Sidebar = () => {
  ...
  const sortBy = [
    { icon: <IndianRupee size={16} />, title: "Earliest Departure" },
    { icon: <Clock size={16} />, title: "Price" },
    { icon: <Hourglass size={16} />, title: "Shortest ride" },
  ]
      ...
      <div className="px-3 py-2">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="mb-2 px-4 text-sm text-primary text-right underline"
          >
            Clear Filter
          </button>
        </div>
      ...
      <div className="px-3 py-2">
        ...
        {departureTime.map((d) => (
          ...
          >
            {d.title}
            <Checkbox
              checked={selectedTimes.includes(d.name)}
              onCheckedChange={() => handleCheckboxChange(d.name)}
              id={d.name}
            />
          </Label>
        ))}
